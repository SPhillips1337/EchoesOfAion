...
export async function fetchFullGameState(gameId: string): Promise<FullGameState> {
    ...
    try {
        // Fetch all entities for the game (assumes game_id column exists on all tables per FN-010)
        ...
        // Placeholder for current turn (would come from games table in future per FN-010)
        const currentTurn = 1;
